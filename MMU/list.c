...
void list_add_ascending_by_address(list_t *l, block_t *newblk){
  ...
  if (curr->next == NULL){
    ...
    if (new->blk->start <= curr->blk->start){
      new->next = l->head;
      l->head = new;
    }
    ...
    if (new->blk->start <= curr->blk->start){
      ...
      while(curr != NULL && new->blk->start > curr->blk->start){
        prev = curr;
        curr = curr->next;
      }
...
void list_add_ascending_by_blocksize(list_t *l, block_t *newblk){
  ...
  if (curr->next == NULL){
    ...
    if(new_blk_size <= curr_blk_size) {
      ...
      while(curr != NULL && new_blk_size > curr_blk_size) {
         prev = curr;
         curr = curr->next;

         if(curr != NULL) 
           curr_blk_size = curr->blk->end - curr->blk->start;
      }
